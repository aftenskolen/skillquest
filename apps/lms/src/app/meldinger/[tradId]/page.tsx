...
export default async function MeldingTradPage({
  params,
}: {
  ...
  if (!trad) redirect("/meldinger");

  const [{ data: meldinger }, { data: laerer }, { data: klasse }] =
    await Promise.all([
      supabase
        .from("melding")
        .select("*")
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 flex flex-col" style={{ height: "calc(100vh - 57px)" }}>
      <div className="mb-4">
        <a href="/meldinger" className="text-sm text-[#1B3A5C] hover:underline">
          ← Tilbake
        </a>
        <h1 className="text-lg font-bold text-[#1B3A5C] mt-1">
          {(laerer as Bruker)?.navn ?? "Lærer"}
        </h1>
